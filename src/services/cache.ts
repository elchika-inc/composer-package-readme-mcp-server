...
export const createCacheKey = {
  ...
  searchResults: (query: string, limit: number, type?: string): string => {
    const queryHash = Buffer.from(query).toString('base64');
    const params = [queryHash, limit.toString()];
    if (type !== undefined) params.push(`t:${type}`);
    return `search:${params.join(':')}`;
  },
  
  downloadStats: (packageName: string): string => 
    `stats:${packageName}:${new Date().toISOString().split('T')[0]}`, // Include date for daily invalidation
};
